...
import { getDirectorTrainingBlockFromSystem } from '../training/context.mjs';
...
export async function callAgent({ tier = 'worker', effort = 'normal', system, messages, maxTokens = 2048, temperature = 0.7 }) {
  ...
  const trainingBlock = tier === 'director' ? getDirectorTrainingBlockFromSystem(system) : '';
  const finalSystem = trainingBlock ? `${system}\n\n${trainingBlock}` : system;

  if (MOCK_AGENT_MODE) {
    const text = buildMockText({ tier, system: finalSystem, messages });
    return {
      text,
      model: `${model}:mock`,
      keyType: 'mock',
